...
exports.addJuzHaliLog = asyncHandler(async (req, res, next) => {
    const { entries, date } = req.body;
    if (!Array.isArray(entries) || entries.length === 0) {
        throw new AppError("entries must be a non-empty array.", 400);
    }
    const today = new Date().toISOString().split("T")[0];
    const count = await service.createJuzHaliLogs(req.user.id, entries, date || today);
    await ThemeModel.incrementStreak(req.user.id);
    res.status(201).json(formatSuccess({ logged: count }, `Logged ${count} Juz Hali entries.`));
});